...
You can set \`throwIfNamespace: false\` to bypass this warning.`);
    ...
    function call(pass, name, args) {
      ...
      if (PURE_ANNOTATION ?? get(pass, "defaultPure")) (0, _helperAnnotateAsPure.default)(node);
      return node;
    }
    ...
    function buildJSXElementCall(path, file) {
      ...
      if (development) {
        args.push(extracted.key ?? path.scope.buildUndefinedNode(), _core.types.booleanLiteral(children.length > 1), extracted.__source ?? path.scope.buildUndefinedNode(), extracted.__self ?? path.scope.buildUndefinedNode());
